...
/**
 * AWS EC2 instance catalog (Linux on-demand, USD/hr). Shared across regions —
 * us-west-2 (Oregon) on-demand pricing matches us-east-1 for these families.
 */
const AWS_EC2: [string, string][] = [
...
const CLOUD_PRICES: SeedPrice[] = [
  ...
  ...catalogRows(CloudProvider.AWS, 'us-east-1', 'EC2', HOUR, AWS_EC2),
  ...
  // ── AWS EC2 — us-west-2 (Oregon; same on-demand pricing as us-east-1) ──
  ...catalogRows(CloudProvider.AWS, 'us-west-2', 'EC2', HOUR, AWS_EC2),
  ...catalogRows(CloudProvider.AWS, 'us-west-2', 'S3', GB_MONTH, [
    ['Standard Storage', '0.023000'],
  ]),
  ...catalogRows(CloudProvider.AWS, 'us-west-2', 'EBS', GB_MONTH, [
    ['gp3 SSD', '0.080000'],
    ['io2 SSD', '0.125000'],
  ]),
...
const REFERENCE_DATA: RefType[] = [
  ...
  },
  {
    ...
    name: 'Document Type',
    values: [
      { code: 'REQUIREMENTS', name: 'Requirements / RFP' },
      { code: 'PROPOSAL', name: 'Proposal' },
      { code: 'SOW', name: 'Statement of Work' },
      { code: 'CONTRACT', name: 'Contract / MSA' },
      { code: 'PRICING', name: 'Pricing / Quote' },
      { code: 'ESTIMATE_SUMMARY', name: 'Estimate Summary' },
      { code: 'ARCHITECTURE', name: 'Architecture Document' },
      { code: 'DESIGN', name: 'Design Document' },
      { code: 'DIAGRAM', name: 'Diagram' },
      { code: 'SPREADSHEET', name: 'Spreadsheet / Worksheet' },
      { code: 'PRESENTATION', name: 'Presentation / Deck' },
      { code: 'CORRESPONDENCE', name: 'Email / Correspondence' },
      { code: 'APPROVAL', name: 'Approval / Sign-off' },
      { code: 'INVOICE', name: 'Invoice / Purchase Order' },
      { code: 'COMPLIANCE', name: 'Compliance / Security' },
      { code: 'OTHER', name: 'Other' },
    ],
  },
];
