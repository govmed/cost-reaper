...
- **Result:** Previewed wordmark on teal/white + the K favicon — clean. e2e-safe (home link still resolves "Kerdos" via substring; no asset asserted by name). CI is the gate.
- **Next:** commit → PR → CI-green → merge. Then the still-pending **$ on monetary values** PR.