...
### 2026-06-17 — Docs: refresh in-app Help guides for recent changes (NFR-12)
- **Action:** User: Help Guides need to be current after the many changes. Updated `apps/web/src/lib/help-content.ts`: find-estimates (status filter → sortable columns + rows-per-page; GM queue note), dashboard-overview (by-stage + GM scoping; "no separate status"), approval-workflow (stage is the lifecycle, transition note, GM Return-to-draft reason), roles-permissions (GM + custom roles FR-30, Governance is admin-only), manage-users (GM/custom roles), manage-reference-data (dropped "Estimate Status" example), cloud-prices (sort/paginate + us-west-2). **Added two new guides:** `upload-documents` (Supporting documents card) and `audit-log` (Admin → Audit Log). Mirrored the key additions into USER_GUIDE.md (Supporting documents, stage-is-lifecycle, transition notes, sort/paginate, Audit Log).
- **Result:** web tsc + eslint + format:check PASS; e2e Help test is not count-sensitive. PR next.

### 2026-06-17 — Audit: record IP + (opt-in) location on every change (FR-11)
- **Action:** Item 1 of 2. Capture the caller's IP (proxy-aware via X-Forwarded-For→req.ip) and optional client location on every audit event. Implemented via AsyncLocalStorage request context (`common/request-context.ts` + `request-context.middleware.ts`, wired in app.module) so `AuditService.record()` stamps ip/location without threading the request everywhere. Added `ipAddress`/`location` to AuditEvent (migration `20260617190000_audit_ip_location`) + AuditEventDto + the audit viewer (IP/Location columns). Web: best-effort `captureClientLocation()` on login (navigator.geolocation, opt-in) cached + sent as `X-Client-Location`; `api.request()` attaches it. Privacy note: this is deliberately more PII than NFR-11's baseline — per explicit user request for accountability.
- **Result:** api tsc + 797 tests; web tsc/eslint/format; migration applied + live-verified (CREATE event captured ip + location). PR next.
...
### 2026-06-19 — SOW: in-app template flavor picker (BR-7)
- **Action:** Wired the 4 SOW flavors into the app. `SowFlavor` enum + `SOW_FLAVORS` catalog in types; `flavor` on CreateSowRequest + StatementOfWorkDto; `template_flavor` column on StatementOfWork (migration `20260619130000_sow_template_flavor`). sow.service `flavorBoilerplate()` merges per-flavor section overrides over the ENTERPRISE defaults (CONCISE/PROPOSAL/TIME_MATERIALS tailor solutionOverview/scope/deliverables/paymentTerms/acceptance/changeControl/terms). create() applies the chosen flavor + stores it; toDto returns it. Web: flavor dropdown (with description) on the SOW create row; print header shows the flavor label. Verified the 3 non-default flavors produce distinct boilerplate.
- **Result:** api tsc + 797 tests; web tsc/eslint/format; migration applied + live-verified. PR next.

### 2026-06-19 — SOW print layout is now flavor-aware (BR-7)
- **Action:** Rebuilt SowPrintPage to adapt to sow.flavor. Sections are now data-driven with dynamic numbering; CONCISE trims the heavy sections (customer understanding, timeline, governance, R&R, NFRs, testing, maintenance, risks, change control). Pricing (§) is flavor-specific: TIME_MATERIALS → Team & rates table + Estimated/Monthly-run-rate/Not-to-exceed; PROPOSAL → Subscription & implementation (one-time impl + monthly/annual subscription) + category breakdown + grand/client; CONCISE → lean totals; ENTERPRISE → line items + category + full waterfall. Acceptance/signatures auto-number after the visible sections.
- **Result:** web tsc + eslint + format:check PASS. PR next.

### 2026-06-19 — Lock supporting documents to Draft (FR-24)
- **Action:** User: an estimator must not edit an estimate that is In Review / Approved / Closed. Core content edits already 409 via ensureEditable (verified: settings/labor/assumptions all 409 for estimator on an IN_REVIEW estimate). Gap: document upload/delete were only permission-gated → closed it. documents.service now `ensureEditable` (currentStage.isInitial) on create + remove → 409 when locked; web DocumentsSection gates the upload form + Delete by `canAuthorEstimates(user) && est.editable` (download stays open). Verified doc upload now 409 on a locked estimate.
- **Result:** api + web tsc/eslint/format PASS; live-verified. PR next. (Next: configurable, admin-only upload size limit — default 100 MB.)

### 2026-06-19 — Configurable, admin-only document upload limit (default 100 MB) (FR-26/NFR-16)
- **Action:** User: the limit is 100 MB, should be configurable, admin-only. Added a minimal app-settings mechanism: `AppSetting` key/value table (migration `20260619140000`, seeds documents.maxUploadMb=100), a `settings` module (SettingsService get/set with clamp 1..100 + audit; SettingsController GET open / PUT @RequirePermission('settings.manage')). New permission `settings.manage` in the catalog. `MAX_DOCUMENT_BYTES` hard ceiling 10→100 MB (interceptor cap); documents.service now reads the configured limit at upload (BadRequest "exceeds N MB"). Web: admin **Settings** page (Admin menu) to edit the MB limit; the Supporting-documents card shows the live "Max N MB" + a client-side too-big guard. Updated DOC-06 spec (ceiling 100MB).
- **Result:** api tsc + 797 tests; web tsc/eslint/format; migration applied + live-verified (default 100, estimator PUT 403, admin set 25, >100 rejected). PR next.

### 2026-06-19 — Tests: Settings (upload limit) + SOW flavor contract suites
- **Action:** Added `apps/api/src/modules/settings/settings-card.spec.ts` (24) — upload-limit constants (100 MB ceiling/default), UpdateDocumentUploadLimitRequest + DocumentUploadLimitDto bounds (1..100, int), and the `settings.manage` permission catalog entry. Added `apps/api/src/modules/sow/sow-flavor.spec.ts` (24) — SowFlavor enum, SOW_FLAVORS catalog (keys↔enum), flavor on CreateSowRequest/StatementOfWorkDto, and SowLineItemDto (LABOR/NONLABOR/CLOUD). 48 pure cases, run in the build gate.
- **Result:** 48/48 pass in-container; prettier clean. Catalog regenerated. PR next.

### 2026-06-19 — SOW: Implementation & Maintenance flavor (full structured model) (BR-7)
- **Action:** Added a 5th SOW flavor IMPL_MAINTENANCE with structured sections (user's it-implementation-maintenance-sow.md template). New SOW fields: `slaTiers`/`supportTiers` (JSON), `warrantyDays` (int), `securityCompliance` (text) — migration `20260619150000`. Service: flavorBoilerplate IMPL_MAINTENANCE narrative overrides + `maintenanceDefaults()` seeds P1–P4 SLAs, 2 support tiers, 90-day warranty, security text ONLY for this flavor (others empty); create/toDto/update wired. Web: editor gains an editable Maintenance/SLA/warranty/security card (add/remove SLA + support rows, warranty number, security textarea); print renders SLA table, Support Hours table, Warranty + Security sections when populated, with dynamic numbering (showIf). 5th flavor in SOW_FLAVORS (api+web) + docs/templates/sow-flavors/SOW-FLAVOR-5 (.md+.docx) + README index. sow-flavor.spec extended (24→30; +SLA/support DTOs, 5-flavor catalog).
- **Result:** api tsc + 851 tests; web tsc/eslint/format; migration applied + live-verified (IMPL_MAINTENANCE → 4 SLAs/2 support/90d/security; ENTERPRISE → none). Catalog 889. PR next.

### 2026-06-19 — SOW: 6th flavor — Implementation & Maintenance (Split Pricing) (BR-7)
- **Action:** User provided a near-duplicate template differing only in §15 (Fees) — split into Implementation (one-time) vs Maintenance (annual). Surfaced the duplication; user chose a separate 6th flavor. Added IMPL_MAINT_SPLIT to SowFlavor + SOW_FLAVORS (api+web). Service: it shares the IMPL_MAINTENANCE narrative (overrides.IMPL_MAINT_SPLIT = overrides.IMPL_MAINTENANCE) + the structured maintenance defaults (MAINTENANCE_FLAVORS list) → SLAs/support/warranty/security. Print: a new `splitPricing` branch in renderPricing renders **Implementation (one-time)** (categories oneTime + contingency = total implementation) and **Maintenance (annual)** (categories yearly + contingency = annual total + monthly equivalent) tables, derived from the engine's per-category subtotals; no new schema. Doc SOW-FLAVOR-6 (.md+.docx) + README. sow-flavor.spec 30→32 (6-flavor catalog).
- **Result:** api tsc + 853 tests; web tsc/eslint/format; live-verified (IMPL_MAINT_SPLIT → 4 SLAs/90d + split pricing). Catalog 891. PR next.
