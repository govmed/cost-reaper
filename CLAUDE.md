...
- **In progress:** (between increments) Sprint 4 governance merged via **PR #6** (`7e5fa89`). Awaiting direction on the next post-MVP priority.
- **Next up (post-MVP options):** (1) first real **Prisma migration** to replace the db-push baseline (housekeeping); (2) **PDF/Excel export** (FR-20); (3) **dashboard** (FR-18); (4) **scenarios + versioning** (FR-14/15); (5) **PERT** three-point estimation (FR-13); (6) **live cloud price-API sync** (FR-21a); (7) harden (flip lint/format to blocking, e2e/Playwright, RBAC integration tests). User granted **standing permission** to proceed without micro-approvals (ask only on bigger decisions); Bash fully allow-listed.
