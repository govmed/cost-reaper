...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004). Merged to `main` through PR #14 (… → Sprint 11 reference-data platform → FE-54 SDLC data-driven). User granted **standing permission** to merge/push/advance **and to not pause with questions** ("just do it"). **FE-27 dashboard on `feature/fe27-dashboard`.**
- **Operating mode:** **AUTONOMOUS** — build without pausing for confirmation; **do not end turns asking which feature to do next** — pick the highest-value item and build it; log decisions; escalate only on hard blockers (see Section 0.1).
- **Current sprint / increment:** FE-27 **Dashboard** (FR-18) — `GET /dashboard` + summary page (counts, per-currency totals, by-stage, recent); branch `feature/fe27-dashboard`.
...
- **Done (recent):** **Sprint 11 merged** (PR #13). **FE-54 part 1 built & locally verified** on `feature/fe54-sdlc-dynamic` — SDLC `sdlcPhase` enum→TEXT (data-preserving migration `20260612160000_sdlc_phase_data_driven`, fresh-deploy + no-drift verified), reusable `ReferenceService.assertActiveCode` (cached) wired into estimate line writes, editor dropdowns offer any active phase. Pipeline green (test 43); live smoke: new admin phase "DISCOVERY" usable end-to-end, invalid → 400. **Next: commit → PR → merge.**
- **Done (recent):** **FE-54 part 1 merged** (PR #14). **FE-27 dashboard built & locally verified** on `feature/fe27-dashboard` — `GET /dashboard` + pure tested `summarizeDashboard` (counts, per-currency exact totals, by-stage, recent), engine `sumMoney`, extracted shared `toMappableEstimate`, web Dashboard page + nav/route, e2e extended. Pipeline green (test 47); live smoke (3 estimates, USD 22340). **Next: commit → PR → merge.**
