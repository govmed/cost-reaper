...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004). Merged to `main` through PR #15 (… → reference-data platform → FE-54 SDLC data-driven → FE-27 dashboard). User granted **standing permission** to merge/push/advance **and to not pause with questions** ("just do it"); all Bash + Docker approved. **FE-23 printable summary on `feature/fe23-print-summary`.**
...
- **Current sprint / increment:** FE-23 **Printable summary** (FR-10) — `/estimates/:id/print` read-only print view + Print button; branch `feature/fe23-print-summary`.
...
- **Done (recent, merged):** **FE-54 part 1** (PR #14, SDLC enum→TEXT validated against `SDLC_PHASE` reference) and **FE-27 dashboard** (PR #15, `/dashboard` + `summarizeDashboard` + page). Test suite at 47. **FE-23 printable summary built & verified** on `feature/fe23-print-summary` — `/estimates/:id/print` read-only doc (lines, totals, phase+category breakdowns, assumptions) + Print button + `print:hidden` chrome; reuses the detail payload (no API/migration); pipeline green; e2e extended. **Next: commit → PR → merge.**
