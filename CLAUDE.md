...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004), now at **`69ed86d`**. Merged to `main` through **PR #35** (all 54 features → live cloud fetch → checklist clickable items + rate-card selector → checklist **deep-link to the specific incomplete line** via `ChecklistItemResult.entityIds` (#31) → in-app **Help guide** of 31 meta-tagged step-by-step use cases (#32) → smart-checklist **"How?" fallback + empty-state** deep-link (#33) → **sticky top nav** (#34) → **freshness timestamps** (#35)). **Latest work (PR #35, merged):** Cloud Prices "last pulled" now shows **date + time** (local); FX Rates shows an overall **"Last updated (local)"** date+time + per-row date+time, plus an admin **"Refresh rates"** button backed by a **live FX pull** (frankfurter.app, no-auth, `FX_RATES_ENDPOINT`, pure-tested `1/rate` mapper, 12s timeout + graceful fallback, USD stays 1, audited; `POST /fx-rates/refresh`). Pipeline green (**test 58**), live-verified (refresh → HTTP 201 with fresh rates + moved timestamps). User wants **fully autonomous, continuous** work — **do not stop to ask**; merge/push/advance freely; `settings.local.json` set to `acceptEdits` + blanket `Bash` (user can flip Shift+Tab → bypass-permissions for zero prompts).
