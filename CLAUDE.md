...
- **Current sprint / increment:** **`v1.0.0` SHIPPED** (PR #76 → tag `v1.0.0` on `main` `54cc96f` + GitHub release; first feature-complete release). All 6 package.json + Swagger at `1.0.0`; CHANGELOG has a `[1.0.0] - 2026-06-15` section + fresh `[Unreleased]`. **Now post-1.0 polish, then new features** (per the user). Recent: checklist "To do" state + panel polish (#72/#73), FE-54 enum-label migration (#74/#75), and **real Veridion logo** in header/login (cropped from veridion.com `main-logo-2.png`; `BrandLogo` has onLight/onDark variants; assets `veridion-logo.png` + `veridion-logo-onteal.png`). **Commit identity is now `Tito Morales <support@veridion.com>`** (local git config set; do NOT use the `tito.morales@govmedai.com` override). Backlog was 54/54.
