...
- **Current sprint / increment:** Continuous run, ~50/54 features done. **Next:** FE-31 OWASP hardening, FE-22 PDF/Excel, FE-24 scenarios, FE-25 versioning, FE-12 multi-currency, FE-40 live cloud sync.
...
- **Backlog:** **~50/54 features done** (this run added FE-19 PERT, FE-20 margin/tax, FE-54b status, FE-28 comments). **Left (~7):** FE-31 OWASP hardening, FE-22 PDF/Excel export (needs a dep), FE-24 scenarios, FE-25 versioning/baselines, FE-12 multi-currency/FX, FE-40 live cloud price pull + "last pulled" (FR-21a/b), FE-54 tail (behavioral enums — billing/role/provider — intentionally code-coupled; labels can be reference-driven). Test suite at **50**.
- **Next up (no need to ask):** continue through the remaining post-MVP features, build/verify/PR/merge each; do not pause to ask which next.
