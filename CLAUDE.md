...
- **MVP status:** Sprint 0 (foundation) + Sprint 1 (auth/RBAC/rate-cards/audit) **merged to `main`** ✓. **Sprint 2 in progress** — EP-4 estimates + line items, EP-5 totals endpoint (engine), EP-11 cloud lines + FE-38 cloud-prices read, EP-8 search, EP-6 CSV export. Remaining after: governance UI (EP-12 post-MVP), web UI build-out.
...
- **In progress:** **Sprint 2** (`feature/sprint2-estimates`) — building estimates CRUD + clone, labor/non-labor/cloud line items + assumptions, the engine-backed totals endpoint, cloud-prices read (FE-38), list search/filter, and CSV export. Wires `@cost-reaper/engine` into the API.
- **Next up:** PR #4 (Sprint 2 core) → CI → merge; then web UI build-out (estimate authoring screens) + first real Prisma migration; governance UI (workflow/checklist, EP-12) is post-MVP.
