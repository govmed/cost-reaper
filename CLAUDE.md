...
- **In progress:** **Admin/Reference-data web UI** — user found the gap (no menus for rate cards/users/cloud prices; those were API-only). Phased: **Part 1 = nav + Rate Cards** (create cards, add/inline-edit/delete roles & rates) **merged via PR #9** (`dc51f4d`); local stack rebuilt + serving it. **Part 2 = Users**, **Part 3 = Cloud Prices** still to do. Also pending in web: per-line upcharge/rate override; admin editors for workflow stages + checklist rules.
- **Next up:** Admin UI **Part 2 (Users)** + **Part 3 (Cloud Prices browse)**. Then post-MVP options: PDF/Excel (FR-20), dashboard (FR-18), scenarios/versioning (FR-14/15), PERT (FR-13), live cloud sync (FR-21a). Standing permission to proceed without micro-approvals; Bash/Docker allow-listed.
