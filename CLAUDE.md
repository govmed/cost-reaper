...
- **In progress:** PR #1 (`feature/ep1-foundation` → `main`) — **CI is GREEN ✓** (build + security pass; verified `pnpm install → prisma generate → typecheck → test → build` — CI is the only oracle: no host Node, Docker daemon down locally). Fixed pnpm version/cache clashes + `composite`/`--noEmit`; format/lint advisory. Added **HTML docs site + draw.io flowcharts** (FE-47; Mermaid rejected → draw.io). Awaiting human merge of PR #1 (merging to `main` is human-gated).
