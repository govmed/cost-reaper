...
| **FR-30** | **Admin-defined roles & permissions (configurable RBAC).** Admins can **create new roles, rename and modify existing roles, and activate/deactivate** them **without a code change or redeploy** — extending FR-26/FR-29's "no hard-coding" principle to the role set itself. Authorization moves from role-name checks to a **permission model**: the system ships a fixed, code-defined **catalog of permissions** (capabilities such as `estimate.edit`, `workflow.approve`, `users.manage`), each role is a **DB-stored set of granted permissions** the admin edits in a matrix, and every protected endpoint is gated by the **permission(s) it requires** (`@RequirePermission(...)`), resolved from the caller's role at request time (deny-by-default, server-side — NFR-16). Workflow transitions (FR-24) gate on a **role/permission reference** rather than a fixed enum. Role/permission changes are **audited** (FR-11). **Boundary (by design):** an admin can grant any *existing* permission to a new role and gate workflow steps to it, but **cannot invent brand-new coded behavior** — deep behavioral couplings (the ADMIN super-override, provider→pricing-strategy, billing-period→engine math, severity→gating) remain code-coupled, consistent with the "behavioral enums stay coded" note in §10. Baseline roles (Admin/GM/Estimator/Viewer) ship via idempotent seed. *(Builds on FE-45/FE-46; the permission catalog formalizes the matrix already in `roleCapabilities.ts`.)* | Should (Post-MVP) |
...
| **EP-2 Identity & Access Management** | Identity management & role-based access control. | FE-6 Register/login/logout · FE-7 JWT access+refresh · FE-8 Role-based authorization · FE-9 Password hashing & policy · FE-45 User management (admin: invite/CRUD, role assignment, activate/deactivate) · FE-46 RBAC permission model + per-endpoint server-side guards (deny-by-default) · FE-55 **Admin-defined roles & permissions** (FR-30): permission catalog + DB-stored roles & role→permission matrix, `@RequirePermission` guards, admin Roles editor (create/rename/activate, toggle permissions) |
...
| 12 | Configurable RBAC | FE-55 admin-defined roles & permissions (permission catalog + role/role-permission tables, `@RequirePermission` guards, admin Roles editor) | FR-30, NFR-16 |
...
**Notes:**
- **Database-driven reference data (FR-29, NFR-17):** the generic `ReferenceType` + `ReferenceValue` pair replaces hard-coded enums/constants for every configurable lookup. The app loads active values **ordered by `display_order`**, caches them, and resolves by `code`; validation accepts any **active** value of the relevant type (not a fixed list). Seed scripts populate baseline values idempotently at deploy. **Migration plan (EP-13, FE-54):** Prisma/Zod enums move to `ReferenceValue` rows; line items keep their `code` string but validate against the active reference values. Snapshots on saved estimates remain immutable. **Done:** `SdlcPhase` is migrated — its columns are now `TEXT` validated against `SDLC_PHASE` via `ReferenceService.assertActiveCode` (admin can add a phase with no code change). **Still enums (to migrate / keep code-coupled):** `EstimateStatus`, `BillingPeriod`, `RateUnit`, `CloudProvider`, `CloudPriceUnit`, `NonLaborType`, `Role`, `ChecklistSeverity`, `ChecklistScope`, workflow stage keys — note that *behavioral* enums (billing period → engine math, role → RBAC, provider → pricing strategy, severity → gating) stay code-coupled even as their labels come from reference data, since a new value there needs new behavior. Do not add new hard-coded reference lists without recording debt against FR-29. **Roles are a special case:** **FR-30 (FE-55)** lifts the *role set* and *permissions* out of code into a DB permission model (admins create/modify roles, gate endpoints via `@RequirePermission`), so `Role` becomes data-driven for access purposes — but the deep behavioral hooks above (incl. the ADMIN super-override) stay coded by design.
...
- **Repository:** https://github.com/govmed/cost-reaper (codename *cost-reaper*; **product = Kerdos by Veridion LLC**) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004), now at **`75fe0b4`**, merged through **PR #69**. The product is **feature-complete**: the full MVP plus every post-MVP roadmap item ships on `main` across ~69 CI-green PRs. User wants **fully autonomous, continuous** work — do not stop to ask; merge/push/advance freely. **Lesson:** run **ESLint** locally too (not just prettier+tsc) before pushing — the flat config errors on `eslint-disable` directives for unregistered rules.
- **Recently merged to `main` (latest session):** estimate-governance repos — **workflow + transition repo** (#54: `WF-`/`TR-` system keys + admin CRUD at `/workflows`→`/workflows/:id`) and **checklist rule-sets repo** (#55: `ChecklistRuleSet` `RS-` keys grouping rules, admin CRUD; default set drives `evaluate()`); **Statement of Work → official PDF** (BR-7) — `StatementOfWork` (`SOW-…` number) composed from an **approved** estimate, **Issue** locks+snapshots pricing / **Revert** reopens, print-to-PDF with signature blocks; list `/sow` → editor `/sow/:id` → print `/sow/:id/print` (#56; #63 visible save feedback; #64 approved-only source, server-enforced); **rebrand to Kerdos by Veridion LLC** — grouped top nav (3 links + 4 dropdowns: Pricing/Governance/Admin/Docs) and the official amber Veridion "V" logo in header/login/favicon (#57–#62); **smart-checklist honesty** — per-line rules report **"N/A" (nothing to check yet)** instead of a vacuous green, "↻ Re-check" button + "last checked" time, so a fresh estimate no longer looks complete and its real blockers stay red (#65, #67, e2e #68); **changelog rolled up** through #65 (#66); **web cache headers** — no-cache `index.html` + immutable hashed assets in nginx (#69).
- **Operating mode:** **AUTONOMOUS** — build without pausing for confirmation; **never end a turn to ask which feature is next or for routine approval** — pick the highest-value item, build/verify/PR/merge, then continue to the next; escalate only on a true hard blocker.
- **Latest merge (`main` `4971b98`, PR #91):** **GM (General Manager) role** (FR-2/FR-26) — a 4th RBAC role that **reviews/approves estimates and returns them to draft but cannot create/edit**. Added `GM` to the Prisma `Role` enum (migration `20260617060000_add_gm_role`, `ALTER TYPE ADD VALUE`) + shared Zod `Role` + web `Role` type; capability matrix `roleCapabilities.ts` (GM = view/export/use-reference/dashboard + advance-workflow, no authoring/admin) so the Roles & permissions page renders a GM column; UsersPage/WorkflowPage fallback role lists + seeded `ROLE` reference values gain GM; default workflow re-gated so **IN_REVIEW→APPROVED ("Approve") and IN_REVIEW→DRAFT ("Return to draft") require GM** (ADMIN still overrides), and the transition endpoint `@Roles` now admits GM. Estimate mutation endpoints stay ADMIN/ESTIMATOR-only (GM can't edit); GET routes open to any authed user (GM reviews). All CI green (build/e2e/security). **Gotcha hit again:** `gh pr merge` failed locally with `bad object refs/Icon?` (macOS Icon-ref breakage) — but the squash had already landed server-side; recovered local sync via `find .git/refs -name 'Icon*' -delete && git remote prune origin` then `git pull --ff-only`.
- **Current sprint / increment:** **`v1.0.1` being cut** (post-1.0 UI polish). Versions bumped `1.0.0`→`1.0.1` (6 package.json + Swagger); CHANGELOG `[Unreleased]`→`[1.0.1] - 2026-06-16` + fresh Unreleased + compare link refs. Flow: release-prep PR → CI → merge → tag `v1.0.1` → GitHub release. **v1.0.0** was the first feature-complete release (PR #76, tag on `54cc96f`). The v1.0.1 batch (all CI-green, merged): checklist "To do" + panel polish (#72/#73), FE-54 enum-label migration (#74/#75), rate-card $ + 2-decimals + sortable columns (#78), labor add-row alignment (#79), floating Help button (#80), **Veridion wordmark only — orange "V" mark removed everywhere + teal "K" favicon** (#81; `BrandLogo` onLight/onDark → `veridion-wordmark*.png`), and **app-wide `$` money formatting** via shared `apps/web/src/lib/money.ts` `formatMoney`/`currencySymbol` (#82). **Commit identity is `Tito Morales <support@veridion.com>`** (local git config set; do NOT use the `tito.morales@govmedai.com` override). Backlog was 54/54.
...
- **Key capabilities (all shipped on `main`):** multi-cloud pricing **AWS/GCP/Azure + SaaS** over a categorized seeded catalog (Compute/Storage/Networking/Database/Containers/Analytics/AI&ML/Security/Management + managed services & SaaS, FR-21) **+ live admin-triggered price sync (FR-21a) and per-provider "last pulled" freshness (FR-21b)** — `PricingProvider` strategies (AWS SigV4 / Azure Retail / GCP Billing) in `cloud-pricing/pricing-providers.ts`, `POST /cloud-prices/sync` (admin), `GET /cloud-prices/last-pulled`, web Cloud Prices page with "Refresh prices" + last-pulled table; **upcharge %** global + per-line (FR-22); **monthly & yearly** costing (FR-23); **PERT three-point** (FR-13); **margin/tax** (FR-16); **scenarios** (FR-14) + **versioning/baselines** (FR-15); **comments/sharing** (FR-19); **multi-currency / FX** (FR-12/FR-17, `fx` module); **resource allocation & capacity** (FR-27 — resource=100%/day, %-split, ≤100%/date, save-time guard + BLOCKER stage gate); **cost per SDLC phase** (FR-28); **export** CSV + native `.xlsx` (hand-written OOXML, no dep — `estimates/xlsx.ts`) + printable/PDF summary (FR-10/FR-20); **dashboard** with stage drill-down (FR-18); **DB-driven reference data** (FR-29 — generic `reference_type`/`reference_value` + parent-child, reference API, admin UI, 16 seeded types; SDLC phase + estimate status migrated off enums → TEXT validated against the reference table); **identity + RBAC** deny-by-default (FR-26, NFR-16) **+ config-driven SSO** OIDC/SAML/WS-Fed (default `SSO_PROTOCOL=LOCAL`, ADR-0008, `auth/sso`) **+ Roles & permissions page**; **customizable workflow engine** (FR-24) + **smart checklist** (FR-25, `workflow/checklist.service.ts`) gating transitions, both with admin repos/editors (EP-12); **Statement of Work** (BR-7, `sow` module); **high modularity** (NFR-15); **HTML docs + draw.io flowcharts** (FE-47).
- **MVP status:** **COMPLETE + governance** ✅ — Sprints 0–4 merged to `main` (PR #1–#6), all CI-green. Backend: auth/RBAC, user mgmt, rate cards, AWS/GCP/Azure catalog, estimates CRUD+clone, labor/non-labor/cloud lines + assumptions, engine-backed totals (upcharge→contingency, monthly/yearly), search, CSV export, audit, Swagger, **+ workflow engine (FR-24) + smart checklist (FR-25) gating transitions**. **Web app**: login, estimates list/search, estimate editor (line items + live totals + CSV + **governance panel**). Runs via `setup.sh`+`start.sh`. **First real Prisma migration `0_init` shipped** (PR #7) — verified to `migrate deploy` cleanly; db-push baseline retired; `docs/DATABASE.md` added. (All post-MVP epics that were "still open" here — PDF/Excel, scenarios, dashboard, PERT, live cloud price-API sync, hardening — have since shipped.)
...
- **Migration history on `main` (`apps/api/prisma/migrations`):** `0_init` → resource_capacity_sdlc_phase → reference_data_platform → sdlc_phase_data_driven → pert_three_point → margin_tax → estimate_status_data_driven → comments → scenarios → baselines → fx_rates → enterprise_catalog_category → saas_provider → cloud_line_category → workflow_transition_repo → checklist_rule_sets → statement_of_work. All additive / no-drift; each verified `migrate deploy` clean. Snapshots on saved estimates stay immutable.
- **Backlog:** **55/55 features done.** **FE-55 / FR-30 — admin-defined roles & permission-based RBAC — BUILT** (ADR-0010): the Prisma `Role` enum was dropped; roles are a data-driven `roles` table (`code`/`displayName`/`permissions String[]`/`isBuiltin`); `User.role` + `WorkflowTransition.allowedRole` are now `String` validated server-side against active roles (FR-29 pattern); a code-defined **permission catalog** (`packages/types/permissions.ts`) gates writes via `@RequirePermission` + a `PermissionsGuard` (cached role→perms resolver; ADMIN holds wildcard `*`); all ~10 controllers migrated off `@Roles` (legacy `RolesGuard` kept inert as defense-in-depth); `/roles` CRUD + `/permissions` catalog (`roles` module); seed creates the 4 built-ins (ADMIN=`*`, GM=`workflow.advance`, ESTIMATOR=author+sow+advance, VIEWER=none); web **Roles editor** (create/toggle-permissions/activate/delete) + Users/Workflow role dropdowns sourced from `/roles`; `RoleCode`=UPPER_SNAKE string; +36 FR-30 tests. Migration `20260617130000_configurable_roles` (enum→text + drop type + create `roles`). Remaining: FE-54 label-migration polish only (optional).
- **Local running instance (2026-06-17):** Docker stack rebuilt to current `main` (GM live) **with data preserved** — 58 estimates intact; default workflow re-gated (Approve→GM, Return-to-draft→GM) via targeted SQL (fresh seed would do this). **Demo users created** (admin via API): `admin@example.com` [ADMIN, seeded], `gm@example.com` [GM], `estimator@example.com` [ESTIMATOR], `viewer@example.com` [VIEWER] — the three new ones share demo password `Kerdos!2026`.
- **Per-card test initiative (2026-06-17, user-requested "~500 tests, ≥50/card, + a 500-estimate volume test"):** **510 pure vitest cases** across 10 card suites in `apps/api/src/modules/**/*-card.spec.ts` (+ `volume.spec.ts`), each importing the real `@cost-reaper/types` contracts and `@cost-reaper/engine` math — no DB/HTTP, runs in the standard `build` gate. Coverage: Labor 78 (#94), Engine/Totals 56, Settings 52, Non-labor 52, Cloud 52, Assumptions/Comments/Reference/FX 54, Rate card 52, Users/RBAC 52 (incl. GM), Workflow 52, Volume 10 (500-line + 200-line recalc + 500-estimate batch + 500-line capacity sweep, NFR-1/NFR-2). All verified green in-container (`vitest run` + `tsc`). **Verify locally with Docker:** mount the spec(s) into the `api` image and `pnpm exec vitest run` (toolchain isn't on host PATH).
- **Next up (no need to ask):** maintenance/polish or whatever the user directs; build/verify/PR/merge each; do not pause to ask which is next.
...
- **Blockers / risks:** none open. CI: `build` (format→lint→typecheck→test→build, **all blocking**) + `e2e` (Docker images built, full stack up, migrate+seed, Playwright smoke) + `security` — all green on the last merge (#69).
- **Local env note:** node/pnpm/corepack are **not** on PATH in the non-interactive tool shell, so the toolchain (pnpm/turbo tests, build) **can't be run locally here** — rely on **GitHub Actions CI** as the gate before/after merge. The app still runs with only Docker. Gotcha: a stray macOS `Icon\r` file can land in `.git/refs/heads/chore/` and break `git pull` ("fatal: bad object … Icon?"); fix with `find .git/refs -name 'Icon*' -delete && git remote prune origin`.
