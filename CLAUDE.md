...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004), now at **`4ad4b88`**. Merged to `main` through **PR #37** (all 54 features → checklist deep-link (#31) → Help guide (#32) → checklist "How?" fallback (#33) → sticky nav (#34) → freshness timestamps (#35) → docs (#36) → **Workflow authoring UI FR-24/FE-43** (#37: admin /workflow page + CRUD on stages/transitions)). **Latest work (on `feature/checklist-rule-authoring`, not yet merged):** **Checklist-rule authoring UI (FR-25/FE-44)** — admin **/checklist-rules** page + admin CRUD (`GET/POST/PATCH/DELETE /checklist-rules`) to toggle/re-tune built-in rules (severity/active/description) and add/delete custom advisory rules; each rule carries a `hasLogic` flag (built-in evaluator vs always-pass advisory, via exported `EVALUATOR_KEYS`); built-ins undeletable, custom keys validated lower_snake_case; audited; Help use case `manage-checklist-rules`; no migration. Pipeline green (**test 58**); live CRUD+guards verified + seeded rules restored (9, resource_capacity→BLOCKER). **Both engines (workflow + checklist) now have admin surfaces.** User wants **fully autonomous, continuous** work — **do not stop to ask**; merge/push/advance freely; `settings.local.json` set to `acceptEdits` + blanket `Bash` (user can flip Shift+Tab → bypass-permissions for zero prompts).
