...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004). Merged to `main` through **PR #31** (all 54 features → live cloud fetch → smart-checklist clickable items + rate-card selector → checklist **deep-link to the specific incomplete line** via `ChecklistItemResult.entityIds`). **Latest work:** in-app **Help guide** — 31 **meta-tagged** step-by-step use cases (featureIds/route/checklistKeys/keywords) across 6 categories, each with a stable deep-link anchor `/help#uc-<id>`; searchable page + hash-scroll highlight + per-card meta; the smart checklist shows a **"How?"** link on each failing item → the use case explaining the fix. Pipeline green (test 56), live-verified, on `feature/help-use-cases`. User wants **fully autonomous, continuous** work — **do not stop to ask**; merge/push/advance freely; `settings.local.json` set to `acceptEdits` + blanket `Bash` (user can flip Shift+Tab → bypass-permissions for zero prompts).
