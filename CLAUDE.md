...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004), now at **`70894ad`**. Merged to `main` through **PR #36** (all 54 features → checklist **deep-link to the incomplete line** (#31) → **Help guide** of 31 meta-tagged use cases (#32) → checklist **"How?" fallback + empty-state** (#33) → **sticky top nav** (#34) → **freshness timestamps** (#35) → docs sync (#36)). **Latest work (on `feature/workflow-authoring`, not yet merged):** **Workflow authoring UI (FR-24/FE-43)** — exposed the configurable WorkflowEngine through a new admin **/workflow** page + admin CRUD endpoints (`POST/PATCH/DELETE /workflows/default/stages|transitions`): add/rename/reorder/flag stages (single-initial enforced; delete guarded vs in-use/history), add/edit/delete transitions (from→to, role, label, requiresChecklistPass; dup/self-loop guarded), audited; Help use case `manage-workflow` added; no migration. Pipeline green (**test 58**); live CRUD round-trip verified + seeded workflow restored. User wants **fully autonomous, continuous** work — **do not stop to ask**; merge/push/advance freely; `settings.local.json` set to `acceptEdits` + blanket `Bash` (user can flip Shift+Tab → bypass-permissions for zero prompts).
