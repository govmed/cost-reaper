...
- **LaborLineItem** (id, estimate_id→Estimate, rate_card_role_id→RateCardRole, **resource_name nullable** (the assigned human resource), quantity, units `NUMERIC`, rate_snapshot `NUMERIC(18,4)`, **upcharge_percent_override `NUMERIC(5,2)` nullable**, **allocation_percent `NUMERIC(5,2)` default 100** (FR-27), **start_date / end_date `DATE` nullable** (allocation window), **billing_period[ONE_TIME|MONTHLY|YEARLY]**, line_total `NUMERIC(18,4)`)
...
- **Next up:** Admin UI **Part 2 (Users)** + **Part 3 (Cloud Prices browse)**. Then post-MVP options: **resource allocation/capacity (FR-27 — resource=100%, %-split, ≤100%/date)**, PDF/Excel (FR-20), dashboard (FR-18), scenarios/versioning (FR-14/15), PERT (FR-13), live cloud sync (FR-21a). Standing permission to proceed without micro-approvals; Bash/Docker allow-listed.
