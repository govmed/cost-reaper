...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004), now at **`7196b56`**. Merged through **PR #42** (… → Native `.xlsx` (#39) → Dashboard stage drill-down (#40) → reference-data enum labels FE-54 (#41) → **Config-driven SSO** OIDC/SAML/WS-Fed FR-26/ADR-0008 (#42)). SSO is **off by default**; `resolveSsoConfig(env)` dispatcher picks the protocol; OIDC fully functional, SAML/WS-Fed redirect+parse+best-effort sig check; `/auth/sso[/login|/callback]`; first SSO sign-in provisions a VIEWER. Both governance engines have admin surfaces (/workflow, /checklist-rules). **Merged since:** Roles & permissions page FR-2/NFR-16 (PR #43 — /roles menu, 13-capability ✓/✕ matrix mirroring server guards). **Merged since:** SSO LOCAL/OIDC/SAML/WSFED identity switch (PR #44 — `SSO_PROTOCOL=LOCAL` default uses built-in identity; `forceSso` hides the password form). **Merged since:** SSO + LOCAL switch + Roles & permissions + online User Guide (PRs #42–#45). **Latest work (on `feature/expand-cloud-catalog`, not yet merged):** **Expanded cloud catalog (FR-21)** — seeded compute catalog grown ~10 → **180** rows (AWS 78 / GCP 52 / Azure 50) covering all general-purpose/compute/memory/storage/accelerated(GPU) families per provider + storage + alt regions; `catalogRows` builder; extended GCP live-fetch specs. Pipeline green (**test 69**); re-seeded & verified 180 prices. **Merged since:** compute catalog expansion (#46) + nav reorder/Title-Case (#47). **Latest work (on `feature/enterprise-cloud-catalog`, not yet merged):** **Categorized enterprise cloud catalog (FR-21)** — added `CloudPrice.category` + a `GB` unit (migration `20260614000000`, additive/drift-free); ~80 new non-compute rows across **9 categories** (Compute/Storage/Networking/Database/Containers & Serverless/Analytics/AI & ML/Security & Tools/Management & Monitoring) for AWS/GCP/Azure; Cloud Prices page Category filter+column; cloud-line picker grouped by category. Pipeline green (**test 69**); live: **256** prices / 9 categories / 5 units. **Latest (on `feature/managed-saas-catalog`, not yet merged):** **Managed Services + SaaS** — new `SAAS` provider (migration `20260614010000`, additive/drift-free) + 16 managed-service + 26 SaaS rows → **298** prices / **11** categories; sync guards SAAS; cloud-usage field relabeled Usage/mo; verified a SaaS line (M365 ×25) = \$900/mo. User wants **fully autonomous, continuous** work — **do not stop to ask**; merge/push/advance freely; `settings.local.json` set to `acceptEdits` + blanket `Bash` (user can flip Shift+Tab → bypass-permissions for zero prompts).
- **Governance repos (new, autonomous run):** **(A) Workflow + transition repo (FR-24)** merged in **PR #54** — `WorkflowDefinition`/`WorkflowTransition` each got a system key (`WF-`/`TR-`) + description; admin CRUD at `/workflows` (repo) → `/workflows/:id` (stage/transition editor); migration `20260614030000` (data-preserving backfill, no-drift). **(B) Checklist rule sets repo (FR-25)** — on `feature/checklist-rule-sets`, **PR pending**: new `ChecklistRuleSet` (system key `RS-`, label, description, isDefault/isActive) groups `ChecklistRule`s via new `rule_set_id` FK; migration `20260614040000` (table + default `RS-DEFAULT` + backfill all rules + NOT NULL + FK — **fresh-deploy + no-drift verified in Docker**, seed → RS-DEFAULT w/ 9 rules); admin CRUD at `/checklist-rule-sets` → `/checklist-rules/:id` (rule editor scoped to a set); `evaluate()` scopes to the default set (behavior-preserving). API+web typecheck + prettier clean locally. **Pattern for both:** system-assigned key behind the scenes + user label/description; add/update/delete **ADMIN-only**. B merged in **PR #55**. **(C) Statement of Work → official/legal PDF (BR-7)** — on `feature/statement-of-work`, **PR pending**: new `StatementOfWork` entity (system `SOW-xxxxxx` number, status DRAFT/ISSUED, editable sections parties/overview/scope/deliverables/timeline/payment-terms/assumptions/T&C + effective date) composed from an estimate; migration `20260614050000` (additive new table; fresh-deploy + no-drift verified in Docker); SOW module injects `EstimatesService` (now exported) to pull/snapshot pricing; **Issue** locks + snapshots totals (immutable), **Revert** reopens; new **Statements of Work** menu item → list/create-from-estimate (`/sow`) → editor (`/sow/:id`) → official print-to-PDF view (`/sow/:id/print`, signature blocks); create/edit/issue is Admin/Estimator, Viewers read-only. C **merged in PR #56** (after fixing a CI ESLint error — an `eslint-disable` for the unconfigured `react-hooks/exhaustive-deps` rule — and an ambiguous e2e `getByText('Pricing')` selector). **All three requested features merged to `main` (A #54, B #55, C #56); local `main` at `f59326e`.** **Lesson:** run **ESLint** locally too (not just prettier+tsc) before pushing — the flat config errors on `eslint-disable` directives for unregistered rules. Settings: the permission-writer strips bare `Bash`/`Bash(cd *)` blanket allow rules on each rewrite, so they don't persist; `Bash(cd *)` is re-added but for zero prompts the user should toggle **Shift+Tab → bypass-permissions**. Now running bare commands (no `cd` prefix) since the working dir persists.
...
- **Current sprint / increment:** **FEATURE BACKLOG COMPLETE — all 54 FE items shipped** across **28 CI-green PRs** to `main`. This continuous run added FE-19 PERT, FE-20 margin/tax, FE-54b status, FE-28 comments, FE-24 scenarios, FE-25 baselines, FE-40 cloud refresh+last-pulled, FE-31 hardening, FE-22 Excel, FE-12 multi-currency. Now maintenance/polish.
...
- **Key capabilities:** multi-cloud compute pricing **AWS/GCP/Azure** (seeded catalog, FR-21); **upcharge %** global + per-line override (FR-22); **monthly & yearly** costing (FR-23); **resource allocation & capacity** (FR-27 — resource=100%/day, %-split, ≤100%/date, save-time guard + BLOCKER stage gate); **cost per SDLC phase** (FR-28 — per-phase rollup on the detail card + CSV); **database-driven reference data** (FR-29 — generic `reference_type`/`reference_value` + parent-child, reference API, admin UI, 16 seeded types; **SDLC phase fully migrated off its enum → TEXT column validated against the reference table** (admin can add a phase with no code change); **remaining enum columns** — status/billing/role/provider/etc. — still to migrate (rest of FE-54)); **identity management + RBAC** (FR-26, NFR-16); **customizable workflow engine** (FR-24) + **smart checklist** (FR-25) gating transitions (EP-12); **dashboard** (FR-18 — counts, per-currency totals, by-stage, recent activity); **high modularity** (NFR-15); **HTML docs + draw.io flowcharts** (FE-47). **Spec'd, not yet built:** cloud price pull + per-provider "last pulled" (FR-21a/b).
...
- **Done (recent, merged):** FE-54 part 1 (PR #14), FE-27 dashboard (PR #15), FE-23 print (PR #16), FE-11 categories (PR #17), FE-36/37 docs (PR #18). **FE-19 PERT built & verified** on `feature/fe19-pert` — labor 3-point units (opt/likely/pess) → effective units = PERT `(o+4m+p)/6`; engine `pert` + test (48 total), migration `20260613090000_pert_three_point` (fresh-deploy + no-drift), editor inputs. Live smoke: 2/4/12 → units 5, total 1050; invalid ordering → 400. **Next: commit → PR → merge, then continue (FE-20 margin/tax, FE-54 status, FE-22 PDF, scenarios…).**
- **Backlog:** **54/54 features done.** Test suite at **51**. Only optional polish remains: a true `.xlsx`/server-PDF via a dep (Excel currently HTML-table `.xls`; PDF via printable summary); live provider-API price fetch behind the existing `PricingProvider` stub (FR-21a); FE-54 behavioral-enum *labels* from reference data (billing/role/provider stay code-coupled by design); admin authoring UIs for workflow stages + checklist rules.
- **Next up (no need to ask):** maintenance/polish or whatever the user directs; build/verify/PR/merge each; do not pause to ask which next.
