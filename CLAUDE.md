...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004). Merged to `main` through PR #16 (… → FE-54 SDLC data-driven → FE-27 dashboard → FE-23 print summary). User granted **standing permission** to merge/push/advance **and to not pause with questions** ("just do it"); **all Bash approved** (blanket `Bash` allow in `.claude/settings.local.json`) + Docker. **FE-11 governed categories on `feature/fe11-governed-categories`.**
...
- **Current sprint / increment:** FE-11 **Governed cost categories** (FR-29) — non-labor category is a `COST_CATEGORY` dropdown validated server-side (`assertActiveDisplayName`); branch `feature/fe11-governed-categories`.
...
- **Done (recent, merged):** **FE-54 part 1** (PR #14, SDLC enum→TEXT), **FE-27 dashboard** (PR #15), **FE-23 printable summary** (PR #16). Test suite at 47. **FE-11 governed cost categories built & verified** on `feature/fe11-governed-categories` — non-labor category now a `COST_CATEGORY` dropdown + server `assertActiveDisplayName` (reference cache refactored to serve codes+names); no migration. Pipeline green; live smoke: "Licenses" 201, "BogusCat" 400, admin-added "Marketing" instantly usable 201. **Next: commit → PR → merge.**
