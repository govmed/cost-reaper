...
- **MVP status:** Sprints 0–2 **merged to `main`** ✓ (PR #1–#4). **Backend MVP essentially complete**: auth/RBAC, user mgmt, rate cards, AWS/GCP/Azure cloud catalog, estimates CRUD+clone, labor/non-labor/cloud line items + assumptions, **engine-backed totals** (upcharge→contingency, monthly/yearly), search, CSV export, audit, Swagger. **Remaining for MVP: the web UI build-out** (estimate-authoring screens) + first real Prisma migration. Governance UI (workflow/checklist, EP-12) is post-MVP.
...
- **In progress:** (between increments) Sprint 2 merged via **PR #4** (`8c78b88`). Next planned increment: the **web UI** (login, estimates list/search, estimate editor with line items + live totals, CSV export button) talking to the REST API.
- **Next up:** Sprint 3 — web UI build-out (React + TanStack Query/Table + RHF/Zod) consuming the API; generate the first real Prisma migration; then post-MVP (workflow/checklist UI, PDF/Excel, scenarios). Standing permission granted to merge/push without asking.
