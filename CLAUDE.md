...
| **FR-21a** | The system can **synchronize prices from provider pricing APIs** (AWS Price List API, Azure Retail Prices API, GCP Cloud Billing Catalog API) to refresh the catalog. | Should (Post-MVP) |
...
| **EP-5 Estimation Engine** | Calculations. | FE-17 Line/category/grand totals · FE-18 Contingency % · FE-19 Three-point/PERT *(post-MVP)* · FE-20 Margin & tax *(post-MVP)* · FE-41 Upcharge % (global + per-line override) · FE-42 Monthly & yearly (annualized) rollups |
...
| FR-21, FR-21a | EP-11 | FE-38, FE-39, FE-40 |
| FR-22 | EP-5 | FE-41 |
| FR-23 | EP-5 | FE-42 |
...
- **LaborLineItem** (id, estimate_id→Estimate, rate_card_role_id→RateCardRole, quantity, units `NUMERIC`, rate_snapshot `NUMERIC(18,4)`, **upcharge_percent_override `NUMERIC(5,2)` nullable**, **billing_period[ONE_TIME|MONTHLY|YEARLY]**, line_total `NUMERIC(18,4)`)
- **NonLaborLineItem** (id, estimate_id→Estimate, category, description, type[FIXED|RECURRING], amount `NUMERIC(18,4)`, **upcharge_percent_override `NUMERIC(5,2)` nullable**, **billing_period[ONE_TIME|MONTHLY|YEARLY]**, periods, line_total `NUMERIC(18,4)`)
- **CloudComputeLineItem** (id, estimate_id→Estimate, cloud_price_id→CloudPrice, provider[AWS|GCP|AZURE], region, service, sku_or_instance, quantity, **usage_hours_per_month `NUMERIC`** (or usage units), **unit_price_snapshot `NUMERIC(18,6)`**, **upcharge_percent_override `NUMERIC(5,2)` nullable**, **billing_period[ONE_TIME|MONTHLY|YEARLY]** default MONTHLY, line_total `NUMERIC(18,4)`)
...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004). Merged to `main`: **PR #1** foundation, **PR #2** Sprint 1 (auth/RBAC/rate-cards), **PR #3** multi-format diagrams. User granted **standing permission** to merge/push/advance without asking each time. Sprint 2 on `feature/sprint2-estimates`.
...
- **Current sprint / increment:** Sprint 2 — Estimate Authoring + Engine + Export (EP-4, EP-5, EP-6, EP-8, EP-11)
...
- **MVP status:** **COMPLETE** ✅ — Sprints 0–3 merged to `main` (PR #1–#5), all CI-green. Backend: auth/RBAC, user mgmt, rate cards, AWS/GCP/Azure catalog, estimates CRUD+clone, labor/non-labor/cloud lines + assumptions, engine-backed totals (upcharge→contingency, monthly/yearly), search, CSV export, audit, Swagger. **Web app**: login, estimates list/search, estimate editor (line items + live totals + CSV). Runs via `setup.sh`+`start.sh`. Remaining housekeeping: first real Prisma migration (db-push baseline today). Post-MVP epics not yet built: governance UI (workflow FR-24/checklist FR-25), PDF/Excel, scenarios, dashboard, live cloud price sync.
...
- **In progress:** (MVP complete — between increments) Sprint 3 web UI merged via **PR #5** (`ce06e1c`). Awaiting direction on post-MVP priorities.
- **Next up (post-MVP options):** (1) **Governance** — `WorkflowEngine` + `ChecklistEngine` API/UI (FR-24/FR-25, the user's earlier asks; engines seeded but no endpoints yet); (2) first real **Prisma migration** to replace the db-push baseline; (3) PDF/Excel export, dashboard, scenarios, live cloud price sync. User granted **standing permission** to proceed without micro-approvals (ask only on bigger decisions); Bash fully allow-listed.
...
- **Blockers / risks:** none open. CI on PR #1 confirms the foundation **builds clean + tests pass** ✓. Follow-ups (non-blocking): format/lint are **advisory** (no local toolchain to autofix ~90 files — flip to blocking after a `pnpm format` pass); first **Prisma migration not yet generated** (using `db push` baseline); Docker **image build** not yet exercised by CI (only the Node pipeline) — `setup.sh` exercises it on a real Docker host.
