...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004). Merged to `main` through **PR #30** (all 54 features → live cloud fetch → smart-checklist clickable items + rate-card selector). **Latest work:** checklist items **deep-link to the specific incomplete line** (`ChecklistItemResult.entityIds`; editor scrolls to + amber-flashes the `line-<id>` rows, section-anchor fallback) — pipeline green (test 56), live-verified, on `feature/deep-link-checklist-lines`. **Next:** meta-tagged in-app **Help guide** of step-by-step use cases, deep-linkable per use case. User wants **fully autonomous, continuous** work — **do not stop to ask**; merge/push/advance freely; `settings.local.json` set to `acceptEdits` + blanket `Bash` (user can flip Shift+Tab → bypass-permissions for zero prompts).
