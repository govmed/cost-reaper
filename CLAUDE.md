...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004), now at **`baad309`**. Merged through **PR #41** (… → Workflow authoring (#37) → Checklist-rule authoring (#38) → Native `.xlsx` (#39) → Dashboard stage drill-down (#40) → reference-data enum labels FE-54 (#41)). Both engines have admin surfaces (/workflow, /checklist-rules). **Latest work (on `feature/sso-config-driven`, not yet merged):** **Config-driven SSO — OIDC/SAML/WS-Fed (FR-26, ADR-0008)** — pure `resolveSsoConfig(env)` dispatcher (`SSO_ENABLED`+`SSO_PROTOCOL` picks one; fails closed) + pluggable `SsoProvider` (OIDC fully functional dependency-free; SAML/WS-Fed redirect+assertion-parse+best-effort sig check); `/auth/sso` status, `/auth/sso/login` (302→IdP), `/auth/sso/callback`→`AuthService.ssoLogin` provisions least-privilege VIEWER + issues JWTs; web "Sign in with …" button + `/sso/callback`. **Off by default**; password login intact. Pipeline green (**test 69**); live-verified SAML+OIDC dispatch (OIDC discovered Google's authorize URL). **Two follow-on asks queued:** (a) role can/cannot-do capability feature, (b) online User Guide in the menu. User wants **fully autonomous, continuous** work — **do not stop to ask**; merge/push/advance freely; `settings.local.json` set to `acceptEdits` + blanket `Bash` (user can flip Shift+Tab → bypass-permissions for zero prompts).
