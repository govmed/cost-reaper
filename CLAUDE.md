...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** workflow (ADR-0004). **PR #1 (EP-1 foundation) merged** ✓ (`d1bb05d`). Sprint 1 work on `feature/sprint1-auth-ratecards` → PR #2.
...
- **Current sprint / increment:** Sprint 1 — Identity & Reference Data (EP-2 auth/RBAC, EP-3 rate cards)
...
- **MVP status:** Sprint 0 (EP-1 foundation) **merged to `main`** ✓. Sprint 1 (EP-2 auth/RBAC + EP-3 rate cards + FE-30 audit) **built**, on PR #2. Remaining MVP: EP-4 estimate authoring, EP-5 engine endpoints, EP-11 cloud lines, EP-6 export, EP-8 search.
...
- **In progress:** **Sprint 1** on PR #2 — auth module (register/login/refresh/logout/me, argon2 + JWT), deny-by-default RBAC guards (`JwtAuthGuard`+`RolesGuard`) + `@Roles`/`@Public`/`@CurrentUser` + `ZodValidationPipe`, admin user management, rate-card CRUD, audit trail; unit tests for AuthService + ZodValidationPipe. Awaiting CI; merge is human-gated.
- **Next up:** merge PR #2; **Sprint 2** — EP-4 estimates + line items (labor/non-labor/cloud), EP-5 engine totals endpoint (upcharge/contingency/monthly/yearly), FE-39 cloud lines, FE-21 CSV export, FE-26 search; plus generate the first real Prisma migration and a cloud-prices read endpoint (FE-38).
