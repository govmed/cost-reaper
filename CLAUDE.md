...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004). Merged to `main` through PR #11 (foundation → Sprints 1–4 → migration → hardening → admin UI). User granted **standing permission** to merge/push/advance without asking each time. **Sprint 10 on `feature/sprint10-phases-capacity-gates`** (FR-27/FR-28/gates/e2e).
...
- **Current sprint / increment:** Sprint 10 — Resource capacity (FR-27) + SDLC-phase costs (FR-28) + stage gates + e2e (branch `feature/sprint10-phases-capacity-gates`)
...
- **Key capabilities:** multi-cloud compute pricing **AWS/GCP/Azure** (seeded catalog, FR-21); **upcharge %** global + per-line override (FR-22); **monthly & yearly** costing (FR-23); **resource allocation & capacity** (FR-27 — resource=100%/day, %-split, ≤100%/date, save-time guard + BLOCKER stage gate); **cost per SDLC phase** (FR-28 — per-phase rollup on the detail card + CSV); **identity management + RBAC** (FR-26, NFR-16); **customizable workflow engine** (FR-24) + **smart checklist** (FR-25) gating transitions (EP-12); **high modularity** (NFR-15); **HTML docs + draw.io flowcharts** (FE-47). **Spec'd, not yet built:** cloud price pull + per-provider "last pulled" (FR-21a/b); **database-driven reference data** (FR-29/NFR-17/EP-13, ADR-0007) — Sprint 11.
...
- **In progress:** **Sprint 10 built & locally verified** on `feature/sprint10-phases-capacity-gates` — FR-27 resource capacity (engine `findCapacityViolations`, save-time 400 guard, `resource_capacity` BLOCKER rule + seed, labor schema fields), FR-28 SDLC-phase rollup (engine `phases`, detail card, CSV), stage gates wired, web editor updated, Playwright e2e extended. Migration `20260612120000_resource_capacity_sdlc_phase` verified (fresh-DB deploy + no drift). Full pipeline green in a clean container (format/lint/typecheck/test=40/build) **and** live API smoke confirmed (capacity 400, phase totals, persisted fields, rule). **Next: commit → PR → CI-green → merge.**
- **Next up:** **Sprint 11 — Reference Data Platform (FR-29/NFR-17/EP-13):** generic `reference_type`/`reference_value` tables (code/name/desc/order/active/audit + parent-child), reference API + admin CRUD, seed scripts, migrate existing enums (incl. the interim `SdlcPhase`) off hard-coding. Also open: FR-21a/b cloud price pull + "last pulled"; PDF/Excel (FR-20), dashboard (FR-18), scenarios/versioning (FR-14/15), PERT (FR-13). Standing permission to proceed without micro-approvals; Bash/Docker allow-listed.
