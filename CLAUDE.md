...
- **In progress:** (between increments) **Admin/Reference-data web UI COMPLETE** — nav + **Rate Cards** (PR #9), **Users** (PR #10), **Cloud Prices browse** (PR #11), all merged & CI-green; local stack rebuilt + serving all three. Web now covers estimates + governance + full admin. Remaining web niceties: per-line upcharge/rate override; admin editors for workflow stages + checklist rules.
- **Next up:** **Build the spec'd post-MVP requirements** the user added — **FR-27** resource allocation/capacity (resource=100%, %-split, ≤100%/date) and **FR-28** cost-per-SDLC-phase on the detail card — or other post-MVP (PDF/Excel FR-20, dashboard FR-18, scenarios/versioning FR-14/15, PERT FR-13, live cloud sync FR-21a). Standing permission to proceed without micro-approvals; Bash/Docker allow-listed.
