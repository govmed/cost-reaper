...
- **In progress:** (between increments) Prisma migration shipped via **PR #7** (`d2f7dd1`). Awaiting direction on the next priority.
...
- **Blockers / risks:** none open. CI confirms all increments **build clean + tests pass** ✓. Follow-ups (non-blocking): format/lint are **advisory** (no local toolchain to autofix — flip to blocking after a `pnpm format` pass); Docker **image build** not yet exercised by CI (only the Node pipeline) — `setup.sh` exercises it on a real Docker host. (Prisma migration resolved — `0_init` shipped.)
