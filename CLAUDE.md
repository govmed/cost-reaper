...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004). Merged to `main` through PR #17 (… → FE-27 dashboard → FE-23 print summary → FE-11 governed categories). User granted **standing permission** to merge/push/advance **and to not pause with questions** ("just do it"); **all Bash approved** (blanket `Bash` allow in `.claude/settings.local.json`) + Docker. **FE-36/37 docs on `feature/fe36-fe37-docs`.**
...
- **Current sprint / increment:** FE-36/FE-37 **Docs** — `docs/USER_GUIDE.md` + `docs/RUNBOOK.md` (+ README Documentation section); branch `feature/fe36-fe37-docs`.
...
- **Done (recent, merged):** **FE-54 part 1** (PR #14), **FE-27 dashboard** (PR #15), **FE-23 print summary** (PR #16), **FE-11 governed categories** (PR #17). Test suite at 47. **FE-36/37 docs built** on `feature/fe36-fe37-docs` — `docs/USER_GUIDE.md` (end-user walkthrough) + `docs/RUNBOOK.md` (operate/deploy/backup/restore/troubleshoot) + README Documentation section; prettier-clean. **Next: commit → PR → merge.**
