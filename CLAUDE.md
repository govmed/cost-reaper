...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004), now at **`1243467`**. Merged to `main` through **PR #40** (all 54 features → … → **Workflow authoring UI** (#37) → **Checklist-rule authoring UI** (#38) → **Native `.xlsx` export** (#39) → **Dashboard stage drill-down** FR-18 (#40)). **Both engines (workflow + checklist) have admin surfaces** (/workflow, /checklist-rules). **Latest work (on `feature/enum-reference-labels`, not yet merged):** **Reference-data labels for behavioral enums (FE-54 tail, FR-29/NFR-17)** — new reusable `useRefLabeler(typeCode)` hook (`apps/web/src/lib/refLabels.ts`, code-fallback) renders DB-driven display labels for **billing period** (One-time/Monthly/Yearly) and **cloud provider** (Amazon Web Services/…) across the estimate editor + printable summary; stored values stay codes, so behavior stays code-coupled but an admin rename in /reference-data flows to the UI with no code change. Pipeline green (**test 64**); labels live-verified. Remaining enums (role/severity/scope/unit) are now one-liner follow-ons. User wants **fully autonomous, continuous** work — **do not stop to ask**; merge/push/advance freely; `settings.local.json` set to `acceptEdits` + blanket `Bash` (user can flip Shift+Tab → bypass-permissions for zero prompts).
