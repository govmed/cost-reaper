...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004). Merged to `main` through PR #13 (foundation → Sprints 1–4 → migration → hardening → admin UI → Sprint 10 capacity/phases/gates → Sprint 11 reference-data platform). User granted **standing permission** to merge/push/advance **and to not pause with questions** ("just do it"). **FE-54 on `feature/fe54-sdlc-dynamic`** (SDLC phase fully data-driven).
...
- **Current sprint / increment:** FE-54 (part 1) — **SDLC phase fully data-driven** (enum→TEXT, validated against the `SDLC_PHASE` reference table; editor offers any active phase); branch `feature/fe54-sdlc-dynamic`.
...
- **Next up:** **FE-54 part 2** — same pattern for the remaining descriptive enums (estimate status, cost category as governed list, priority/resource-type/document-type if surfaced); behavioral enums (billing period, role, cloud provider, checklist severity) keep code coupling but can read labels from reference data. Then the small cluster (FE-11 governed categories, FE-23 print view, FE-36 user guide, FE-37 runbook) and post-MVP (FR-21a/b last-pulled, FR-20 PDF/Excel, FR-18 dashboard, FR-14/15 scenarios/versioning, FR-13 PERT). Standing permission; Bash/Docker allow-listed; do not pause to ask which next.
