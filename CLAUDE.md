...
- **Backlog:** **54/54 features done.** Remaining is **optional polish only:** FE-54 — migrate the *labels* of the still-behavioral enums (billing period / role / cloud provider / checklist severity) to reference data, noting these stay code-coupled by design (a new value needs new behavior: billing→engine math, role→RBAC, provider→pricing strategy, severity→gating).
