...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004). Merged to `main` through PR #18 (… → FE-23 print → FE-11 categories → FE-36/37 docs). User wants **fully autonomous, continuous** work — **do not stop to ask**; merge/push/advance freely; `settings.local.json` set to `acceptEdits` + blanket `Bash` (user can flip Shift+Tab → bypass-permissions for zero prompts). **FE-19 PERT on `feature/fe19-pert`.**
...
- **Current sprint / increment:** FE-19 **Three-point/PERT** (FR-13) — optional optimistic/most-likely/pessimistic units on labor lines → effective units = PERT; branch `feature/fe19-pert`.
...
- **Backlog:** ~43/54 features done. Left: FE-54 part 2 (status + descriptive enums), FE-31 OWASP, FE-12 multi-currency, FE-19✓, FE-20 margin/tax, FE-22 PDF/Excel, FE-24 scenarios, FE-25 versioning, FE-28 collaboration, FE-40 live cloud sync.
- **Next up (no need to ask):** FE-54 part 2 (remaining descriptive enums + governed cost categories FE-11), then the small cluster (FE-23 print view, FE-36 user guide, FE-37 runbook), then post-MVP (FR-21a/b last-pulled, FR-20 PDF/Excel, FR-14/15 scenarios/versioning, FR-13 PERT). Standing permission; Bash/Docker allow-listed; do not pause to ask which next.
