...
- **In progress:** (between increments) Hardening shipped via **PR #8** (`ad4a369`) — lint/format now **blocking** in CI, **Playwright e2e** + a CI **`e2e` job** that builds the Docker images + runs the full stack (verified locally e2e). Committed `pnpm-lock.yaml` (reproducible installs). Awaiting direction on the next priority.
- **Next up (post-MVP options):** **PDF/Excel export** (FR-20); **dashboard** (FR-18); **scenarios + versioning** (FR-14/15); **PERT** three-point estimation (FR-13); **live cloud price-API sync** (FR-21a); more RBAC integration tests. User granted **standing permission** to proceed without micro-approvals (ask only on bigger decisions); Bash/Docker allow-listed.
