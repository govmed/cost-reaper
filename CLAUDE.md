...
- **Current sprint / increment:** **Cutting `v1.0.0`** — first tagged release (feature-complete: MVP + all post-MVP). Versions bumped `0.1.0`→`1.0.0` (all 6 package.json + Swagger `setVersion`), CHANGELOG `[Unreleased]`→`[1.0.0] - 2026-06-15` (+ fresh empty Unreleased + compare link refs). Flow: release-prep PR → CI-green → merge → annotated tag `v1.0.0` → GitHub release. **Then begin new feature work** (per the user). Backlog was 54/54 before this; recent session shipped checklist "To do" state + panel polish (PRs #72/#73) and the FE-54 enum-label migration (PRs #74/#75).
