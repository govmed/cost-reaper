...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004), now at **`d2378a2`**. Merged to `main` through **PR #38** (all 54 features → checklist deep-link (#31) → Help guide (#32) → checklist "How?" fallback (#33) → sticky nav (#34) → freshness timestamps (#35) → docs (#36) → **Workflow authoring UI** FR-24/FE-43 (#37) → **Checklist-rule authoring UI** FR-25/FE-44 (#38)). **Both engines (workflow + checklist) now have admin surfaces** (/workflow, /checklist-rules). **Latest work (on `feature/native-xlsx-export`, not yet merged):** **Native `.xlsx` export (FR-20/FE-22)** — replaced the HTML-table `.xls` with a real OOXML workbook via a **dependency-free** writer (`apps/api/.../estimates/xlsx.ts`: CRC32 + ZIP/deflate using Node `zlib` + the 5 SpreadsheetML parts; numeric cells as numbers, else inline strings). Shared `exportRows()` between CSV+XLSX; correct content-type + `.xlsx` filename. Pipeline green (**test 64**, +6 xlsx unit tests); live-verified the downloaded file is a valid OOXML workbook (python zipfile integrity OK, all parts present, data round-trips). User wants **fully autonomous, continuous** work — **do not stop to ask**; merge/push/advance freely; `settings.local.json` set to `acceptEdits` + blanket `Bash` (user can flip Shift+Tab → bypass-permissions for zero prompts).
