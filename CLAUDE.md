...
- **Repository:** https://github.com/govmed/cost-reaper  (project: *cost-reaper*) — **`main` is the single trunk** (unprotected); **trunk-based** (ADR-0004), now at **`7cf3534`**. Merged to `main` through **PR #39** (all 54 features → … → **Workflow authoring UI** FR-24/FE-43 (#37) → **Checklist-rule authoring UI** FR-25/FE-44 (#38) → **Native `.xlsx` export** FR-20/FE-22 (#39, dependency-free OOXML writer)). **Both engines (workflow + checklist) have admin surfaces** (/workflow, /checklist-rules). **Latest work (on `feature/dashboard-stage-drilldown`, not yet merged):** **Dashboard "By workflow stage" drill-down (FR-18)** — each stage row is now a toggle that expands an inline list of the estimates in that stage (name link, grand total, currency, updated), backed by `GET /dashboard/stage/:stageKey` (`DashboardService.estimatesInStage`; `UNASSIGNED`→null stage; engine-computed totals) + `DashboardStageEstimate` type + `useStageEstimates` hook. Pipeline green (**test 64**); live-verified (DRAFT→13 estimates, unknown stage→empty). User wants **fully autonomous, continuous** work — **do not stop to ask**; merge/push/advance freely; `settings.local.json` set to `acceptEdits` + blanket `Bash` (user can flip Shift+Tab → bypass-permissions for zero prompts).
